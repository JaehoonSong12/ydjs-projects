...
function on_v8 {
    ...
    if [[ -d "$HOME/.nvm" ]]; then
        echo "Notice: $(USRPROMPT 'NVM (Node Version Manager) old version is already installed at $HOME/.nvm.')"
        rm -rf "$HOME/.nvm"
    fi
    echo "Installing NVM (Node Version Manager)..."
    curl -o "$PWD/ext/nvm-setup.sh" "https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.1/install.sh"
    bash "$PWD/ext/nvm-setup.sh"  # Run the NVM installation script
    rm "$PWD/ext/nvm-setup.sh"  # Clean up the NVM installation
    echo "$(USRPROMPT 'NVM has been reset.')"
...
function SETUP_NODE {
    ...
    echo -e "NODEJS"
    echo -e "------------------------------------------------------------\n"
    echo -e "Node.js version: \t\t\t$(node -v)\t@$(which node)" 
    echo -e "Node Package Manager (npm) version: \t$(npm -v)\t\t@$(which npm)"
    echo -e "Node Package eXecutor (npx) version: \t$(npx -v)\t\t@$(which npx)"
    echo -e "NVM version: \t\t\t\t$(nvm --version)\t\t@$NVM_DIR"
    # < npx >
    # to execute Node.js binaries or scripts directly from the 
    # command line without globally installing them
    echo -e "------------------------------------------------------------\n"
    echo -e "To install Node.js LTS version, use:"
    echo -e "  $(USRPROMPT 'nvm install lts/*')" # Install the latest LTS version of Node.js
}
...
# JDK_URL="https://download.java.net/java/ga/jdk11/openjdk-11_windows-x64_bin.zip" # version-11
function on_jvm {
