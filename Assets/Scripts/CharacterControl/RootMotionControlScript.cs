...
public class RootMotionControlScript : MonoBehaviour
{
    ...
    private void Update()
    {
        if (cinput.enabled)
        {
            ...
            // modify #3: Scale the forward input to maintain the 70% ratio for walking
            if (_inputForward > 0)
            {
                _inputForward = walkingSpeedRatio + (1f - walkingSpeedRatio) * _inputForward;
            }
            else
            {
                _inputForward = walkingSpeedRatio * _inputForward;
            }
        ...
        // modify #3: Set the Animator's speed
        anim.speed = animationSpeed;
    }
    // modify #3: maintained a ratio of 60% for the running animations for SomeDude_RootMotion.
    public float animationSpeed = 1f; // New variable
    public float rootMovementSpeed = 1f; // New variable
    public float rootTurnSpeed = 1f; // New variable
    public float walkingSpeedRatio = 0.6f; // New variable for controlling the walking speed ratio
    ...
    void FixedUpdate()
    {
        ...
        if (_inputActionFired)
        {
            ...
            if (buttonDistance <= buttonCloseEnoughForMatchDistance)
            {
                ...
                else
                {
                    ...
                    // modify #6: Added match target and inverse kinematics for button press animation in SomeDude_RootMotion
        ...
        // modify #6: get info about current animation
    ...
    void OnAnimatorMove()
    {
        ...
        // TODO (done): Scale the root motion delta
        ...
        // modify #3: Apply the scaled root motion delta to the transform
